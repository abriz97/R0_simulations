...
for (name in names){
  ...
  for(c in 1:(ncol(df - 1))){
    ...
    if (somecases){
      ...
      epestim <- epiestim(incidence[1:14,], mean_GT = mean_si, sd_GT = sd_si)
      ...
      eglin <- EG_Lin(data = incidence[1:14,], mean_GT = mean_si, sd_GT = sd_si)
      ...
      ## Mine as well
      epestim2 <- epiestim_andrea(data = incidence[1:14,], mean_GT = mean_si, sd_GT = sd_si, andrea = TRUE)
...
oo <- ggplot(coverage_stats, aes(x = method, col = rho, fill = rho)) +
  geom_errorbar(aes(ymin = q_Rt05, ymax = q_Rt95), position = "dodge", width = 1, alpha=.5) +
  geom_point(aes(y = q_Rt50), position = position_dodge(width = 1)) + 
  geom_hline(aes(yintercept = as.numeric(gsub("R0 = ", "", R0))), linetype = 'dashed')+
  facet_grid(cols = vars(R0)) + ylab('R0')
  labs(fill = "Reporting \nrate", col = 'Reporting \nrate') 
