...
# ## Test
# simulate_seir_stochastic(arnaught = 2.0, t_E = 4, t_I = 4, Npv = 1e6, S_init = Npv-10, E_init = 0, I_init = 10, n_t = 1000, n_steps_per_t = 10) -> stochastic
# simulate_seir_ode(arnaught = 2.0, t_E = 4, t_I = 4, Npv = 1e6, S_init = Npv-10, E_init = 0, I_init = 10, n_t = 1000, n_steps_per_t = 10) -> ode
# 
...
# 
# 
# lapply(1:1000, function(xx) simulate_seir_example(arnaught = 2.0, t_E = 4, t_I = 4, Npv = 1e6, E_init = 0, I_init = 10, n_t = 200, n_steps_per_t = 10, method = 'stochastic')) %>%
#   bind_rows() %>%
#   group_by(time) %>%
#   summarise_all(mean) -> stochastic
#   
# simulate_seir_example(arnaught = 2.0, t_E = 4, t_I = 4, Npv = 1e6, E_init = 0, I_init = 10, n_t = 200, n_steps_per_t = 10, method = 'ode') -> ode
# bind_rows(stochastic, ode, .id = c('method')) %>%
#   mutate(method = factor(method, labels = c('stochastic', 'ode'))) %>%
#   pivot_longer(S:dIR) %>%
#   ggplot()+
