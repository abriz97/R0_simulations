...
simulate_data <- function(N, gamma, sigma){
  ...
  # randomly select values of R0, population size (Np), and initial number infected (I)
  ...
  for(i in 1:N){
    ...
    # run the SEIR model
    out <- deSolve::ode(y=state_init, times=timestep, func=model, parms=pars, method="rk4") 
...
simulate_data_missed_generations <- function(N, gamma, sigma, generations = 0){
